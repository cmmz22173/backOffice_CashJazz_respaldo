"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GuardiaService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const guardia_entity_1 = require("./guardia.entity");
let GuardiaService = class GuardiaService {
    constructor(guardiaRepository) {
        this.guardiaRepository = guardiaRepository;
    }
    async createGuardia(Guardia) {
        const guardiaFound = await this.guardiaRepository.findOne({});
        if (guardiaFound) {
            return new common_1.HttpException('Guardia already exists', common_1.HttpStatus.CONFLICT);
        }
        const newGuardia = this.guardiaRepository.create(Guardia);
        return this.guardiaRepository.save(newGuardia);
    }
    async getGuardia(id) {
        const guardiaFound = await this.guardiaRepository.findOne({
            where: {
                id
            }
        });
        if (!guardiaFound) {
            return new common_1.HttpException('guardia Guardia not found', common_1.HttpStatus.NOT_FOUND);
        }
        return guardiaFound;
    }
    async getGuardias() {
        const guardiasFound = await this.guardiaRepository.find();
        if (!guardiasFound) {
            return new common_1.HttpException('Turnos Guardia not found', common_1.HttpStatus.NOT_FOUND);
        }
        return guardiasFound;
    }
    async updateGuardia(id, guardia) {
        const guardiaFound = await this.guardiaRepository.findOne({
            where: {
                id
            }
        });
        if (!guardiaFound) {
            return new common_1.HttpException('guardia Guardia not found', common_1.HttpStatus.NOT_FOUND);
        }
        const updateTurnoGuardia = Object.assign(guardiaFound, guardia);
        return this.guardiaRepository.save(updateTurnoGuardia);
    }
};
GuardiaService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(guardia_entity_1.guardia)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], GuardiaService);
exports.GuardiaService = GuardiaService;
//# sourceMappingURL=guardia.service.js.map